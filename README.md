...
```
async-worker/
  typescript/creditCheck/v01/actors/checkBureau/checkBureau.ts  # fsmLanguage: "typescript"
  python/creditCheck/v01/actors/checkBureau/checkBureau.py      # fsmLanguage: "python"
  rust/creditCheck/v01/actors/checkBureau/checkBureau.rs        # fsmLanguage: "rust"
  go/creditCheck/v01/actors/checkReportsTable/checkReportsTable.go  # fsmLanguage: "go"
```
...
`generate-async-logic` command into `async-worker/<lang>/` at the **monorepo
root** (run that command first if the directory doesn't exist yet — from the
repo root, `-f apps/fsm-core-example/fsm` is enough; the command always writes
to `Deno.cwd()`, i.e. wherever you ran it from — see
[section 2](#2-scaffold-fsm-operation)). Each connects to the gateway's
...
deno run --allow-all async-worker/typescript/cli.ts start \
  --gateway-socket /tmp/pgfsm-activity-gateway-workers.sock
```

```bash
# Python
cd async-worker/python
...
Run this from the **monorepo root** — at instance-start time `fsmlet` resolves
each FSM's sync/async operation modules via `Deno.cwd()` (`sync-worker/`/
