...
i.e. wherever you ran it from; see [section 2](#2-scaffold-fsm-operation)). Each
connects to the gateway's `--sidecar-socket` above and serves invocations for
every actor compiled into its registry until stopped.
...
Run this from **`apps/`** — at instance-start time `fsmlet` resolves each FSM's
sync/async operation modules via `Deno.cwd()` (`sync-worker/`/ `async-worker/`,
whichever directory it's actually started from), independent of `-f` below
(which is only for `fsmlet`'s own startup validation).
