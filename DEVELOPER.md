...
from the **repo root** (see the table's own `Command` row above), so
`sync-worker/`/`async-worker/` land there, siblings of `apps/`. Their
...
`generate-async-logic` command into `async-worker/<lang>/` at the **repo root**
(run that command first if the directory doesn't exist yet — from the repo root,
`-f apps/fsm-core-example/fsm` is enough; the command always writes to
...
Also run this from the repo root — at instance-start time `fsmlet` resolves each
