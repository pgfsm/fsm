...
def create_app(
    create_fsm_app: Optional[Callable[[asyncpg.Pool], Awaitable[None]]] = None,
) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        if settings.DB_TYPE in ("postgres", "supabase_and_postgres"):
            ...
            try:
                ...
                if create_fsm_app is not None:
                    try:
                        await create_fsm_app(pool)
                    except Exception as exc:
                        logger.error(f"create_fsm_app startup error: {exc}")
