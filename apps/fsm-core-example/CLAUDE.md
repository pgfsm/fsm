...
- `async-worker/<lang>/<fsmName>/<fsmVersion>/actors/` — one subtree per
  ...
  `@pgfsm/async-worker-sdk`; Python/Rust/Go: `cli.py`/`sdk.py` etc.;
...
`requirements.txt` (`async-worker/python/`) — see #318. Like those three, it's
compiler-generated by `generate-async-logic`'s `writeWorkerSdk` step (both
folder mode and single-`fsm.json` mode — see
