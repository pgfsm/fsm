...
Start the FSM Hono server with configurable database URL, URL prefix, and FSM folder paths.
...
FSM folder flags are all optional — omit any you don't need. The server starts without loading any FSM definitions if no paths are given.
...
Send `SIGINT` (Ctrl+C) or `SIGTERM` to stop the server gracefully. Press Ctrl+C a second time to force-exit immediately.
