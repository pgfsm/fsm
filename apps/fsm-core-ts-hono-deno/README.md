...
The HTTP gateway for the FSM framework. Built with [Hono](https://hono.dev/), runs on Deno.
...
The server holds no FSM business logic. It translates HTTP requests into database calls and enqueues events for workers to process asynchronously.
