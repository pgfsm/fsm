...
Deno.test("writeWorkerSdk - removes a stale pre-#370 generated sdk.go, keeps a hand-edited one", async () => {
  for (const handEdited of [false, true]) {
    const dir = await Deno.makeTempDir();
    try {
      const appRootAbsPath = `${dir}/apps/fsm-core-example`;
      const goDir = `${appRootAbsPath}/async-worker/go`;
      await Deno.mkdir(goDir, { recursive: true });
      const content = handEdited
        ? "// my own sdk\npackage main\n"
        : "// AUTO-GENERATED by fsm-compiler-ts. Do not edit directly.\n";
      await Deno.writeTextFile(`${goDir}/sdk.go`, content);
      ...
      if (handEdited) {
        assertEquals(await Deno.readTextFile(`${goDir}/sdk.go`), content);
      } else {
        let statErr: unknown;
        try {
          await Deno.stat(`${goDir}/sdk.go`);
        } catch (err) {
          statErr = err;
        }
        assertExists(statErr, "generated sdk.go should have been removed");
      }
      assertExists(await Deno.stat(`${goDir}/main.go`));
    } finally {
      await Deno.remove(dir, { recursive: true });
    }
  }
