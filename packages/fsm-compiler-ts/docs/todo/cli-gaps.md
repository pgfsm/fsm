...
| `generateFsmJSONFromMachineFile`      | `generate-fsm-json` (when `-f` is a `.ts` file) | ✅                                                                |
...
| `--folder` rejected non-directories for `generate-fsm-json` | stat check block         | ✅ Fixed — `isDirectory` assertion skipped when `command === "generate-fsm-json"`           | Medium   |
...
| `--help` / `-h`                                  | ✅                  |
| no args → help                                   | ✅                  |
| `generate-fsm-json` (success)                    | ✅                  |
| `generate-fsm-json --show-recommendation` / `-r` | ✅                  |
| `generate-async-logic`                           | ✅                  |
