...
Suggestions to improve consistency between source files, exported functions, CLI
...
The file was named `cleanFsmJSON.ts` but exported `deleteFsmJSONFromFolders`.
The CLI command was `clean`. All three used different verbs.

| Layer             | Was                        | Is now                        | Status |
| ----------------- | -------------------------- | ----------------------------- | ------ |
| File name         | `cleanFsmJSON.ts`          | `deleteFsmJSONFromFolders.ts` | ✅     |
| Exported function | `deleteFsmJSONFromFolders` | _(unchanged)_                 | ✅     |
...
`validateFsmPluginLoadFromFolder` and `validatePromisePluginLoadFromFolder`
previously took 7+ parameters all prefixed with `fsm`. The function name already
scopes them.
...
Anonymous object shapes that appeared across multiple files are now named types
in `util.ts` and exported from `index.ts`.
...
All four `validate*` functions in `validateFsmPluginLoad.ts` now have explicit
`Promise<FsmPluginValidationResult>` / `Promise<FsmPluginValidationResult[]>`
return types.
...
All exported functions that process collections use the plural form. This
function processes all `invoke` entries in a JSON tree but was named in the
singular.
...
`validatePromisePluginLoadFromFolders` was exported but had no CLI command while
its FSM counterpart had `validate`.
...
`validate` was ambiguous — it could mean "validate the fsm.json schema" or
"validate the TypeScript plugin module exports". It's the latter.
`validate-plugin` makes the target explicit. The sister command follows the same
