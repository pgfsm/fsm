...
`run-async-worker.ts` + `deno.json` importing `@pgfsm/async-worker-sdk`;
...
| Current status        | Not used — `fsm-async-worker-gateway-ts` doesn't need this step. It's a compile-time model, not a runtime one: each lang ipc worker self-registers its actors with the gateway instead of being validated from folders at startup | Used by `fsm-sync-worker-ts` — `fsmlet` runs `validateSyncOperationFromFolders` on every startup |
