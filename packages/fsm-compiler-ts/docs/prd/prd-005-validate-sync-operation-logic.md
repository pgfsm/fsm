# PRD-005 — Validate sync operation logic

**Package:** `@pgfsm/compiler` (`packages/fsm-compiler-ts`) **Status:**
Partially implemented — see [Gaps](#gaps) and [TODO.md](../todo/TODO.md).
**Related:**
[PRD-003 — Scaffold sync operation logic](./prd-003-scaffold-sync-operation-logic.md),
[PRD-004 — Validate async operation logic](./prd-004-validate-async-operation-logic.md).

## Summary

Once the action / guard / delay stubs scaffolded in PRD-003 are filled in, the
compiler **validates** that each is exported with the right shape. This is part
of §3 ("Validate operation logic") of the root
[`DEVELOPER.md`](../../../../DEVELOPER.md).

## Background

**Sync operation logic** is the set of `actions`, `guards`, and `delays`
referenced in `fsm.json`. Unlike actors (PRD-004), it runs **inline inside a
macrostep** of the `fsmlet` — no separate queue or process.

This stage confirms `fsm.json` follows the machine schema and that every
referenced action/guard/delay is exported as a function.

| Command                   | Function                           | Module                                 |
| ------------------------- | ---------------------------------- | -------------------------------------- |
| `validate-sync-operation` | `validateSyncOperationFromFolders` | `src/validate-sync-operation-logic.ts` |

## Goals

- Validate `fsm.json` against `fsm.machine.schema.v3.json`.
- Validate that every referenced action, guard, and delay is exported as a
  function.

## Non-goals

- Scaffolding the stubs (PRD-003).
- Async actors — validation of async operation logic (PRD-004).
- Running the machine — the `fsmlet` / worker stage.

## Requirements

### R1 — Validate sync modules

```bash
deno run --allow-all packages/fsm-compiler-ts/src/cli/index.ts \
  -c validate-sync-operation \
  -f apps/fsm-core-example/fsm \
  -w fsm
```

`validateSyncOperationFromFolder`:

- validates `fsm.json` against `fsm.machine.schema.v3.json` with Ajv (early
  return with defaults if the schema check fails);
- extracts action/guard/delay/actor refs via `extractFsmPluginRefs`;
- calls `validateLanguageModules` to import
  `<lang>/{actions,guards,delays}/index.ts` (TypeScript only — see
  [Gap 2](#gaps)) and confirm each referenced name is a `function`.

**Status:** ✅ Implemented — `validateSyncOperationFromFolders` /
`validateSyncOperationFromFolder` (`src/validate-sync-operation-logic.ts`),
wired to the `validate-sync-operation` command.

**Note:** the function signature still accepts an `availableActors` list, but
the current implementation does not read it — external-actor dependency
resolution against that list was never wired up. The dead `--available-actors`
CLI flag has been removed (#222); the parameter itself remains on the function
signature since other callers (`fsm-sync-worker-ts`) also pass it, always as
`[]`. See [Gap 3](#gaps).

### R2 — Shape (arity) validation

`validateLanguageModules` confirms each name is a `function` via `typeof` but
does **not** check its arity or signature, so the action/guard stub-signature
mismatches from PRD-003 (gaps 1 and 3) still pass `validate-sync-operation`. See
[Gap 1](#gaps).

## Gaps

Tracked in [TODO.md](../todo/TODO.md):

1. **`typeof`-only validation** — `validateLanguageModules` checks only that a
   name is a `function`, not its arity, so mismatched action `(context, event)`
   and guard `(context, event)` stubs (PRD-003) validate despite not matching
   the worker's `(context, params, meta)` / `(context, cond, meta)` calling
   convention. Validate arity/shape, not just that the export exists.
2. **TypeScript-only module validation** — `validateSyncOperationFromFolder`
   calls `validateLanguageModules(absPath, "typescript", …)` with the language
   hardcoded, so `python`/`rust`/`go` sync stubs generated by
   `generate-sync-logic` are never validated. Validate every language present
   under the folder.
3. **External-actor resolution is a no-op; `availableActors` is now dead
   end-to-end.** `validateSyncOperationFromFolder` and
   `validateSyncOperationFromFolders` both accept and pass through an
   `availableActors` parameter, but nothing in the current implementation reads
   it. The `--available-actors` CLI flag was removed (#222) since nothing wired
   it to real behavior; the parameter itself is left on the signatures because
   `fsm-sync-worker-ts` also calls these functions (always passing `[]`). Either
   wire external-actor checking up for real, or drop the parameter entirely
   (would also require updating `fsm-sync-worker-ts`'s callers).

## Acceptance criteria

- `validate-sync-operation` rejects an `fsm.json` that fails schema validation.
  ✅
- `validate-sync-operation` reports each missing/incorrect action, guard, or
  delay. ✅
- Stubs whose signature does not match the worker's calling convention are
  reported as failures. ❌ pending gap 1.
- Non-TypeScript sync stubs are validated. ❌ pending gap 2.
- External-actor dependencies are checked against the machine's actors. ❌
  pending gap 3 — the `--available-actors` CLI flag was removed as dead plumbing
  rather than wired up.
