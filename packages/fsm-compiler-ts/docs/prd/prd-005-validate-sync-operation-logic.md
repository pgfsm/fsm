...
**Note:** an earlier revision threaded an unused `availableActors` list through
these functions for external-actor dependency resolution, but nothing ever read
it. Both the dead `--available-actors` CLI flag (#222) and the `availableActors`
parameter itself (#224) have since been removed. See [Gap 3](#gaps).
...
2. **TypeScript-only module validation** — `validateSyncOperationFromFolder`
   ...
   under the folder.
3. **External-actor resolution was never implemented.**
   `validateSyncOperationFromFolder` and `validateSyncOperationFromFolders` used
   to accept and pass through an `availableActors` parameter, but nothing ever
   read it, so it was removed entirely (#222, #224) rather than wired up. If
   external-actor dependency checking is needed later, it'll need a new
   parameter and real implementation — there's nothing to resurrect.
...
- External-actor dependencies are checked against the machine's actors. ❌
  pending gap 3 — the dead `availableActors` plumbing was removed rather than
  wired up; this would need new work, not restoration.
