...
- `deno.json` — declares `@pgfsm/sync-worker` as a real `npm:` import so
  ...
  Also carries a `name` (see below), a `description` ("Generated by
  @pgfsm/compiler. Runs this project's FSM sync-operation worker via
  @pgfsm/sync-worker."), an `exports` field pointing at `run-sync-worker.ts`
  (Deno warns that a bare `name` with no `exports` looks like an incomplete
  JSR-publish config — harmless either way here, but this silences the warning
  since nothing about this file is actually meant for JSR), and two `tasks`:
  `dev` (`deno run --allow-all run-sync-worker.ts`) and `dev:watch`
...
`sync-worker-<8 hex chars>` (`crypto.randomUUID().split("-")[0]`) — a name is
always written (never left out) precisely because of the JSR-config warning
above.
