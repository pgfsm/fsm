...
CREATE OR REPLACE FUNCTION fsm_core.select_transitions_with_guard_eval_v2(
...
-- select fsm_core.select_transitions_with_guard_eval_v2('xstate.done.state.(machine).creditCheck.CheckingCreditScores', 'creditCheck', 'v3');
...
-- select fsm_core.select_all_transitions_v2(
...
-- SELECT array_agg(t) INTO all_transition_records FROM fsm_core.select_all_transitions_v2(
-- 'Submit', 
-- ARRAY['machine', 'machine.creditCheck', 'machine.creditCheck.Entering_Information']::TEXT[],
-- 'creditCheck', 
-- 'v2'
...
-- this should be simulated in application layer, we can call fsm_core.select_transitions_with_guard_eval_v2 function to get the valid transitions based on guard evaluation, 
...
-- '{}'::jsonb would be resolved in ARRAY['machine', 'machine.creditCheck', 'machine.creditCheck.Entering_Information'] in fsm_core.fsm_worker_v2, so we can directly pass the resolved value here for testing fsm_core.macrostep_v2
